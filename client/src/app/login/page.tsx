    ...
    <div className="min-h-screen bg-[#0a0a0a] flex flex-col items-center justify-center px-4">
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-[13px] text-[#555]">Sign in to your ChatLax account</p>
        </div>
        ...
        <div className="bg-[#111] border border-[#1f1f1f] rounded-2xl p-6 space-y-4">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <button type="submit" disabled={isLoading}
              ...
              {isLoading ? <><div className="w-3.5 h-3.5 border-2 border-[#0a0a0a] border-t-transparent rounded-full animate-spin" />Signing in...</> : 'Sign in'}
            </button>
          </form>
        </div>
        ...
        <p className="text-center text-[13px] text-[#555] mt-5">
          ...
          <Link href="/register" className="text-[#888] hover:text-[#e0e0e0] transition-colors">Create one</Link>
        </p>
      </motion.div>
    </div>
  );
